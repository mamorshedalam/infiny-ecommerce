...
export default function ProductFilter({ name, data }) {
     ...
     return (
          <div className="border-b mb-12">
               ...
               <ul className={`cardBar overflow-y-scroll mt-2.5 mb-5 ${isCollapsed ? "max-h-60" : "h-0"}`}>
                    {data && data.length > 0 && data.map((selector, index) => (<li><a index={index} href="" className="text-zinc-400 py-1 hover:text-neutral-900 sl-animated-lg">{selector.label}</a></li>))}
               </ul>
          </div>
     )
}